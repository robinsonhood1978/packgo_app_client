...
import DrawerScreen from './src/screens/demos/drawer';
import DemosScreen from './src/screens/demos';
import MultipleImagesScreen from './src/screens/multiple_images';
import TapsScreen from './src/screens/taps';
import SwiperScreen from './src/screens/swiper';
import SqliteScreen from './src/screens/sqlite/HomeScreen';
import RegisterUser from './src/screens/sqlite/RegisterUser';
import UpdateUser from './src/screens/sqlite/UpdateUser';
import ViewUser from './src/screens/sqlite/ViewUser';
import ViewAllUser from './src/screens/sqlite/ViewAllUser';
import DeleteUser from './src/screens/sqlite/DeleteUser';
...
function MainTab() {
  return (
    <Tab.Navigator tabBar={(props) => <TabBar {...props} />}>
      ...
      <Tab.Screen name="DemosStack" component={DemosStack} />
...
function App() {
  ...
  const fetchPublishableKey = async () => {
    ...
    const key =
      'pk_test_51JerMkBKfUykjlb5cSrZBhltlo6H2feeq7HB7c7X1mMH8nX6csqx25uGX4s2m2LTOmLO4lveKNugiUcwI7u6EezR008D6f0qpA';
  ...
  const authContext = React.useMemo(
    () => ({
      getParcels: async ({token}) => {
        const parcels = await services.getParcels(
          {state: 'DPT', count: 2},
          token,
        );
        console.log('robin parcels:', parcels);
        dispatch({type: 'PARCELS', parcels});
      },
      signIn: async ({username, password}) => {
        ...
        try {
          ...
          if (includes(roles, 'packgo_app_client')) {
      ...
      signOut: async () => {
        try {
          ...
          console.log('Robin logout finished');
        } catch (e) {
          console.log(e);
        }
      },
