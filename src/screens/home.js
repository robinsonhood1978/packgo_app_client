...
import {darkOrange, cyan, aqua} from 'src/configs/colors';
import {getDate3} from 'src/utils/time';
import {getSymbol} from 'src/utils/currency_formatter';
...
function HomeScreen(props) {
  ...
  async function fetchData(query, cb = () => {}) {
    try {
      console.log('robin:', userToken);
      // getParcels({token: userToken});
  ...
  return (
    <View
      style={[
        styles.container,
        {backgroundColor: colors.thirdBackground, paddingTop: insets.top},
      ]}>
      <ScrollView
          ...
          <View style={styles.viewTableInfo}>
            <View style={styles.table}>
              <ViewResult
                ...
                nameIcon="cash-usd"
                symbol={symbol}
                symbolOnLeft={positionCurrency === 'left'}
