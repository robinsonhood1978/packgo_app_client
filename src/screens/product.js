...
class Product extends React.Component {
  ...
  fetchProducts = async () => {
    try {
      ...
      const parcels = this?.context?.parcels ?? '';
      console.log('parcels all', parcels.length);
      const data = await services.getSubParcels(query, userToken, parcels);
      if (data.length > 0) {
        console.log('parcels processing:', data.length);
