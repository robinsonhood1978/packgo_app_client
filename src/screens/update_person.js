...
function UpdatePersonScreen(props) {
  ...
  const saveProfile = async () => {
    try {
      setLoading(true);
      const result = await services.updateProfile(data, userToken);
      console.log('robin saveProfile', result);
      if (result.code === 0) {
        showMessage({
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            ...
            <Input label={t('inputs:text_about')} multiline /> */}
              <InputImage
                ...
                value={data?.avatar}
                onChangeImage={(value) => updateData('avatar', value)}
              />
            </View>
          </ScrollView>
